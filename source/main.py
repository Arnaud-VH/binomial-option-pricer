...
pricer_bt = BinomialOptionPricer(
   S0 = S0, K=K, T=1, R = R, n_steps = 10, option_type="call", sigma=vol
)
pricer_bs = BlackSholes(
   S0 = S0, K=K, T=1, R = R, option_type="call", vol=vol
)
vis = Visual(S0 = S0, K=K, T=1, R = R, vol=vol, ticker_input=ticker_input)
